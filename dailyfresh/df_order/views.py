...
from df_user.views import user_decorator
...
'''
django中使用事物的方法：
 在视图函数前加上@transaction.atomic(),在django.db中引入 transaction才能使用
...
def order_handle(request):
    ...
    cart_ids = request.POST.get('cart_id')  # 接受购物车编号
    try:
        # 创建订单对象
        order = Order_Info()
        now = datetime.now()
...
@user_decorator.login
def pay(request, oid):
    order = Order_Info.objects.get(oid=oid)
    order.oIspay = True
    order.save()
    context = {'order': order}
